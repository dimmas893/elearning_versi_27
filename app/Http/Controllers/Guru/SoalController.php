...
class SoalController extends Controller
{
    public function index($category_soal, $jadwal)
    {
        ...
        $category_soal = Category_soal::where('master_category_soal_id', $category_soal_master->id)->get();
        // $mata_pelajaran = MataPelajaran::where('id', $jadwal->mata_pelajaran_id)->get();

        $kelas = Kelas::all();
        // $soal = Soal::with('guru')->get();
        return view('frontend.guru.category_soal.index', compact('kelas', 'category_soal', 'jadwal', 'category_soal_master'));
    }
    ...
    {
        // $jadwal = Jadwal::where('id', decrypt($jadwal))->first();
        $category_soalcategory_soal = Category_soal::where('id', decrypt($category_soal))->firstOrFail();
        $jadwal = Jadwal::where('id', decrypt($jadwal))->firstOrFail();
        $mata_pelajaran = MataPelajaran::where('id', $jadwal->mata_pelajaran_id)->get();
    ...
    public function siswa_soal($semester, $category_soal, $mata_pelajaran_id)
    {
        ...
        // $hasil_soal_salah = Hasil_nilai::where('semester_id', $semestersemester->id)->where('category_soal_id', $category_soal->id)->where('kelas_id', $category_soal->kelas_id)->where('mata_pelajaran_id', $mata_pelajaran->id)->where('siswa_id', Auth::guard('siswa')->user()->id)->where('nilai', 0)->count('nilai');

        return view('frontend.siswa.soal.index', compact('semestersemester', 'category_soal', 'mata_pelajaran', 'soal', 'cek', 'hasil_soal'));
    }
